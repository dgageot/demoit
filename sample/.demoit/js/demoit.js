...
class FakeWindow extends BaseLitElement {
    static get properties() {
        return {
            title: { type: String },
            addTab: { type: Boolean, attribute: 'add-tab' }
        }
    }
    ...
    render() {
        ...
        <div id="main">
            <div id="bar">
                ...
                ${this.title ? html`<span id="title">${this.title}</span>` : ''}
                <slot name="bar"></slot>
            </div>
            <slot></slot>
        </div>`;
...
class SourceCode extends BaseLitElement {
    static get properties() {
        return {
            ...
            files: {
                type: String,
                converter: {
                    fromAttribute: v => { return v.split(' ').filter(n => n.trim() !== ''); }
                }
            },
            startLines: {
                type: String,
                attribute: 'start-lines',
                converter: {
                    fromAttribute: v => { return v.split(';'); }
                }
            },
            endLines: {
                type: String,
    ...
    render() {
        ...
        <fake-window title="code ~ ${this.folder}">
            <div id="tabs">
            ${this.files.map((file, i) => html`<a class="${(i == 0) ? "selected" : ""}" href="#">${file}<span class="close">x</span></a>`)}
            </div>
            <div id="container">
                <div id="source"></div>
            </div>
    ...
    firstUpdated() {
        this.$$('a').forEach((link, index) => link.addEventListener('click', () => this.current = index));
    }
    ...
    async showCurrentTab() {
        ...
        const startLines = this.startLines[this.current];
        const endLines = this.endLines[this.current];
        ...
        this.$$('a').forEach((link, index) => link.classList.toggle('selected', index == this.current));
    }
}
...
class WebBrowser extends BaseLitElement {
    ...
    render() {
        ...
        <fake-window>
            ...
            <iframe id="site" class="site" src="${this.src}"></iframe>
        </fake-window>`;
    }
...
class WebTerm extends BaseLitElement {
    ...
    async addTab() {
        ...
        this.$('.newtab').onclick = () => this.addTab();
    }
}
